...
execute if score @s sloDst matches 1.. run function hp:item_functions/deluminator/ray_to/raycast
scoreboard players operation @s sloDst -= @s sloDstPer
execute if score @s sloDst matches ..0 at @s run function hp:item_functions/deluminator/ray_to/private/end
