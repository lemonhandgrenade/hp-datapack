...
execute as @e[type=armor_stand,tag=broom,tag=broomMain,limit=1,sort=nearest,distance=..2] unless score @s broomType matches 16 unless score @s broomType matches 3 store result entity @s Pose.Head[0] float 0.01 run scoreboard players get @p sloID
execute  