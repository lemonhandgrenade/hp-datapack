...
execute if score @s sloDst matches 1.. run function hp:spells/f/freezing_charm/raycast
scoreboard players operation @s sloDst -= @s sloDstPer
execute if score @s sloDst matches ..0 at @s run function hp:spells/f/freezing_charm/private/end
