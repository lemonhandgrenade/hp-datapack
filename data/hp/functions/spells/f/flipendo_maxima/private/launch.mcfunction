...
execute if score @s sloDst matches 1.. run function hp:spells/f/flipendo_maxima/raycast
scoreboard players operation @s sloDst -= @s sloDstPer
execute if score @s sloDst matches ..0 at @s run function hp:spells/f/flipendo_maxima/private/end
