...
execute if score @s sloDst matches 1.. run function hp:spells/s/shooting_spell/raycast
scoreboard players operation @s sloDst -= @s sloDstPer
execute if score @s sloDst matches ..0 at @s run function hp:spells/s/shooting_spell/private/end
