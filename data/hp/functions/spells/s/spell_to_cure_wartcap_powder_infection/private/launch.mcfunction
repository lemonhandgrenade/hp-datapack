...
execute if score @s slo_dst matches 1.. run function hp:spells/s/spell_to_cure_wartcap_powder_infection/raycast
scoreboard players operation @s slo_dst -= @s slo_dst_per
execute if score @s slo_dst matches ..0 at @s run function hp:spells/s/spell_to_cure_wartcap_powder_infection/private/end
