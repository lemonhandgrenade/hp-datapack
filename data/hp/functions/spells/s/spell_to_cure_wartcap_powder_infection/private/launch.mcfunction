...
execute if score @s sloDst matches 1.. run function hp:spells/s/spell_to_cure_wartcap_powder_infection/raycast
scoreboard players operation @s sloDst -= @s sloDstPer
execute if score @s sloDst matches ..0 at @s run function hp:spells/s/spell_to_cure_wartcap_powder_infection/private/end
