...
#execute if score #temp slowcast matches 0.. as @a unless predicate hp:player/is_sneaking unless predicate hp:player/is_sprinting at @s as @e[type=area_effect_cloud,tag=patronusCharm,tag=slowcast] if score @s UID = @p UID rotated as @p positioned as @s positioned ^ ^ ^0.21585 run function hp:spells/p/patronus_charm/raycast
#execute if score #temp slowcast matches 0.. as @a unless predicate hp:player/is_sneaking if predicate hp:player/is_sprinting at @s as @e[type=area_effect_cloud,tag=patronusCharm,tag=slowcast] if score @s UID = @p UID rotated as @p positioned as @s positioned ^ ^ ^0.2806 run function hp:spells/p/patronus_charm/raycast
#execute if score #temp slowcast matches 0.. as @a if predicate hp:player/is_sneaking unless predicate hp:player/is_sprinting at @s as @e[type=area_effect_cloud,tag=patronusCharm,tag=slowcast] if score @s UID = @p UID rotated as @p positioned as @s positioned ^ ^ ^0.2806 run function hp:spells/p/patronus_charm/raycast
...
execute if score #temp slowcast matches 0.. if entity @p[tag=this,predicate=hp:player/is_sneaking] run scoreboard players add @s slo_dst 1
