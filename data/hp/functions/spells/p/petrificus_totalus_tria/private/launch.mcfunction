...
execute if score @s slo_dst matches 1.. run function hp:spells/p/petrificus_totalus_tria/raycast
scoreboard players operation @s slo_dst -= @s slo_dst_per
execute if score @s slo_dst matches ..0 at @s run function hp:spells/p/petrificus_totalus_tria/private/end
