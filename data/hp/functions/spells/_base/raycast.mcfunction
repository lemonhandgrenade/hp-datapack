...
execute if entity @s[tag=!end] if entity @e[tag=!spellCaster,tag=!this,tag=!slowcast,tag=!lightBlock,tag=!Lock,dx=0] positioned ~-0.7 ~-0.7 ~-0.7 if entity @e[tag=!spellCaster,tag=!this,tag=!slowcast,tag=!lightBlock,tag=!Lock,dx=0] positioned ~0.7 ~0.7 ~0.7 run function hp:spells/_base/hit_entity
...
execute if score @s spellForm matches 2 facing entity @e[type=#hp:alive,tag=!spellCaster,tag=!this,sort=nearest,limit=1,distance=..30] eyes positioned ^ ^ ^10 rotated as @s positioned ^ ^ ^40 facing entity @s eyes facing ^ ^ ^-1 positioned as @s run tp @s ~ ~ ~ ~ ~
