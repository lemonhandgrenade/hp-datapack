...
execute if score @s sloDst matches 1.. run function hp:spells/u/unidentified_durmstrang_students_fire_spell/raycast
scoreboard players operation @s sloDst -= @s sloDstPer
execute if score @s sloDst matches ..0 at @s run function hp:spells/u/unidentified_durmstrang_students_fire_spell/private/end
