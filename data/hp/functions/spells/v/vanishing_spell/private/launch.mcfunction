...
execute if score @s sloDst matches 1.. run function hp:spells/v/vanishing_spell/raycast
scoreboard players operation @s sloDst -= @s sloDstPer
execute if score @s sloDst matches ..0 at @s run function hp:spells/v/vanishing_spell/private/end
