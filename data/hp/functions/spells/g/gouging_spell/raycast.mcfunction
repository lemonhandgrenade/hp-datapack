...
execute if score #temp slowcast matches 0.. if entity @s[tag=!done] if score charmsBreakBlocks settings matches 1 positioned ^ ^ ^0.5 if block ~ ~ ~ #hp:breakable if score charmsChangeBlocks settings matches 1 run function hp:spells/g/gouging_spell/destroy
