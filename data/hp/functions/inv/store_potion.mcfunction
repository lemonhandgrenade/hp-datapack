...
execute positioned ~ -1 ~ run data modify entity @e[type=minecraft:armor_stand,tag=new,tag=potionCarrier,distance=..1,limit=1] HandItems[0] set from storage hp:potion PotionItem
execute at @s positioned ~ -1 ~ run scoreboard players operation @e[type=minecraft:armor_stand,tag=new,tag=potionCarrier,limit=1,sort=nearest] UID = @s UID
execute at @s positioned ~ -1 ~ as @e[type=minecraft:armor_stand,tag=new,tag=potionCarrier,limit=1,sort=nearest] run tag @s remove new
